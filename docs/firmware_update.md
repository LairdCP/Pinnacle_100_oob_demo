...
2. Disconnect the terminal program from the console UART and transfer the update file to the Pinnacle 100 using the mcumgr CLI via the console UART.
    ...
    mcumgr -t 20 -r 3 --conntype serial --connstring dev=/dev/tty.usbserial-A908JLEI,mtu=512 image upload /Users/ryan/Desktop/pinancle100_v3.0.103.bin

    # Windows

    mcumgr -t 20 -r 3 --conntype serial --connstring dev=COM4,mtu=512 image upload C:\pinnacle100_v3.0.103.bin
...
1. Transfer the update file to the Pinnacle 100 using the mcumgr CLI via BLE.

    ```
    mcumgr -t 20 -r 3 --conntype ble --connstring ctlr_name=hci0,peer_name='Pinnacle 100 OOB-0303848' image upload /Users/ryan/Desktop/pinnacle100_v3.0.103.bin
...
2. List the images to obtain the hash of the update image in slot 1
    ...
    ```
    mcumgr --conntype ble --connstring ctlr_name=hci0,peer_name='Pinnacle 100 OOB-0303848' image list
...
3. Test the image in slot 1. This sets the image in slot 1 to be swapped and booted.
    ...
    ```
    mcumgr --conntype ble --connstring ctlr_name=hci0,peer_name='Pinnacle 100 OOB-0303848' image test e378dde02fe58825fe0b620926ec932f0a4aaaa82857e897e40f7486d2011276
...
4. Issue a reset to swap to the slot 1 image and boot it. This can take some time to complete.
    ...
    ```
    mcumgr --conntype ble --connstring ctlr_name=hci0,peer_name='Pinnacle 100 OOB-0303848' reset
...
6. Confirm the image once it has booted. If the new image is not confirmed, the image will be swapped back to slot 1 on the next reboot.
    ...
    ```
    mcumgr --conntype ble --connstring ctlr_name=hci0,peer_name='Pinnacle 100 OOB-0303848' image confirm
