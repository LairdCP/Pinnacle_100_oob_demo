...
void bootloader_fetch(void)
{
	...
	ubs_set_bootloader_present(bootloader_present);
	...
	if (bootloader_present == true) {
		...
		value8 = 1;
#endif
		ubs_set_bootloader_header_checked(value8);
		...
		if (value8 == BOOTLOADER_INIT_STATE_INITIALISED) {
			ubs_set_ext_header_version(value16);
		}

		value8 = BlrPubGetInfo(NULL, &value16, NULL);

		if (value8 == BOOTLOADER_INIT_STATE_INITIALISED) {
			ubs_set_ext_function_version(value16);
		}

		value8 = BlrPubGetInfo(NULL, NULL, date);

		if (value8 == BOOTLOADER_INIT_STATE_INITIALISED) {
			ubs_set_firmware_build_date(date);
		}
		...
		if (status == BOOTLOADER_STORAGE_CODE_SUCCESS) {
			ubs_set_customer_key_set(value8);
		}

		status = BlrPubQuery(
			BOOTLOADER_STORAGE_INDEX_READBACK_PROTECTION, 0, 0,
			&value8, sizeof(value8), NULL, NULL, NULL);
		if (status == BOOTLOADER_STORAGE_CODE_SUCCESS) {
			ubs_set_readback_protection(value8);
		}

		status = BlrPubQuery(
			BOOTLOADER_STORAGE_INDEX_CPU_DEBUG_PROTECTION, 0, 0,
			&value8, sizeof(value8), NULL, NULL, NULL);
		if (status == BOOTLOADER_STORAGE_CODE_SUCCESS) {
			ubs_set_cpu_debug_protection(value8);
		}

		status =
			BlrPubQuery(BOOTLOADER_STORAGE_INDEX_QSPI_CHECKED, 0, 0,
				    &value8, sizeof(value8), NULL, NULL, NULL);
		if (status == BOOTLOADER_STORAGE_CODE_SUCCESS) {
			ubs_set_QSPI_checked(value8);
		}

		status = BlrPubQuery(
			BOOTLOADER_STORAGE_INDEX_BOOTLOADER_RELEASE_BUILD, 0, 0,
			&value8, sizeof(value8), NULL, NULL, NULL);
		if (status == BOOTLOADER_STORAGE_CODE_SUCCESS) {
			ubs_set_bootloader_type(value8);
		}

		status = BlrPubQuery(
			BOOTLOADER_STORAGE_INDEX_BOOTLOADER_UPDATE_FAILURES, 0,
			0, &value8, sizeof(value8), NULL, NULL, NULL);
		if (status == BOOTLOADER_STORAGE_CODE_SUCCESS) {
			ubs_set_bootloader_update_failures(value8);
		}

		status = BlrPubQuery(
			BOOTLOADER_STORAGE_INDEX_BOOTLOADER_UPDATE_LAST_FAIL_CODE,
			0, 0, &value8, sizeof(value8), NULL, NULL, NULL);
		if (status == BOOTLOADER_STORAGE_CODE_SUCCESS) {
			ubs_set_bootloader_update_last_fail_code(value8);
		}

		status = BlrPubQuery(
			BOOTLOADER_STORAGE_INDEX_MODEM_UPDATE_LAST_FAIL_CODE, 0,
			0, &value8, sizeof(value8), NULL, NULL, NULL);
		if (status == BOOTLOADER_STORAGE_CODE_SUCCESS) {
			ubs_set_bootloader_modem_update_last_fail_code(value8);
		}

		status = BlrPubQuery(
			BOOTLOADER_STORAGE_INDEX_COMPRESSION_UPDATE_ERRORS, 0,
			0, &value8, sizeof(value8), NULL, NULL, NULL);
		if (status == BOOTLOADER_STORAGE_CODE_SUCCESS) {
			ubs_set_bootloader_compression_errors(value8);
		}
		...
		status = BlrPubQuery(BOOTLOADER_STORAGE_INDEX_BOOT_VERIFICATION,
				     ...
				     NULL);
		if (status == BOOTLOADER_STORAGE_CODE_SUCCESS) {
			ubs_set_boot_verification(value8);
		}
		...
		if (status == BOOTLOADER_STORAGE_CODE_SUCCESS) {
			ubs_set_customer_key(buffer);
		}

		status =
			BlrPubQuery(BOOTLOADER_STORAGE_INDEX_QSPI_HEADER_SHA256,
				    0, 0, buffer, sizeof(buffer), NULL, NULL,
				    NULL);
		if (status == BOOTLOADER_STORAGE_CODE_SUCCESS) {
			ubs_set_QSPI_sha256(buffer);
		}
		...
		if (status == BOOTLOADER_STORAGE_CODE_SUCCESS &&
			...
			strftime(buffer, sizeof(__DATE__), "%b %d %Y",
				 timesplit);
			buffer[sizeof(__DATE__) - 1] = 0;
			ubs_set_module_build_date(buffer);
		}
	} else {
